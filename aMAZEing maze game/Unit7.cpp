...
void __fastcall TFormRate::FormShow(TObject *Sender)
{
	...
	Label2->Caption = "Ваше время: " + FloatToStr(passingTime) + " секунд";
	Label4->Caption = "Ваша сложность: " + EditCrutches->Text;
}
//---------------------------------------------------------------------------
...
void __fastcall TFormRate::ButtonSaveClick(TObject *Sender)
{
	...
	fstream RatingFile("E:\\Main\\!КУРСОВАЯ РАБОТА ПРОГРАММА\\aMAZEing maze game\\MazeRating.txt", ios_base::app);
	RatingFile << "\n" << difficulty << "\n" << name << "\n" << passingTime;
	RatingFile.close();
}
//---------------------------------------------------------------------------
